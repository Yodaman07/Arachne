...
with (socket.socket(socket.AF_INET, socket.SOCK_STREAM) as s):
    ...
    while cap.isOpened():
        ...
        # Send length first
        s.sendall(struct.pack(">L",
                              len(img_bytes)))  # > L is big-endian format, and L is a long which has a certain size of 4 bytes - can be used for a certain size when unpacking
